...
- Includes your photo in `assets/profile.jpg` (and `assets/IMG-20251221-WA0056.jpg`).
...
1. Make sure the full folder is ready, including `assets/profile.jpg`.
...
Notes
- The image path in `index.html` is set to relative `assets/profile.jpg` with fallbacks so it loads reliably on GitHub Pages.
- Make sure to upload or commit the `assets` folder along with `index.html`.
